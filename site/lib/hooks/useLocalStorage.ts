...
type UseLocalStorageReturnType<T> = {
  value: StateValueState<T>;
  setValue: React.Dispatch<React.SetStateAction<UseLocalStorageReturnType<T>["value"]>>;
  getLocalStorageValue: () => StateValueState<T>;
  setLocalStorageValue: (newVal: T) => void;
};

export const useLocalStorage = <T>({
  initialValue,
  key,
}: UseLocalStorageOptions<T>): UseLocalStorageReturnType<T> => {
  ...
  const setLocalStorageValue = useCallback(
    (newVal: T) => {
      const formattedValue = ["string", "number"].includes(typeof newVal)
        ? newVal
